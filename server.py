...
IMPORT_SYSTEM = (
    ...
    "现有节点清单主要用于边端点引用。\n"
    "- 节点 name 必须是简短主题名（≤15 字，优先中文）：禁止裸公式、禁止书名/页码/章节号引用"
    "（这类信息放 statement 或 tags）。公式只允许作为短修饰出现在 name 里"
    "（如「Bézout 恒等式」可以，「det(A^t)=det(A)」不行——应命名为「转置保持行列式」）。\n"
    "- 边方向：depends_on 的 A→B 表示「B 的证明使用了 A」。"
    "示例：「余子式展开 ⇒ det(A^t)=det(A) 的证明」应连 cofactor-expansion → det-transpose，"
    "而不是反过来。拿不准方向的边宁可不加。\n"
