...
| `position` | object \| null | 画布坐标 `{ "x": 数值, "y": 数值 }`；`null` 表示尚未布局——只要任一节点为 `null`，前端首次加载会自动布局并把坐标写回。分层模式下 `y` 应落在本层 zone 内（zone 高度随层节点数自适应）：拖出 zone 会被钳制回去，`y` 不在自己 zone 内的节点会被视为未布局，自动参加层内网格排列。 |
